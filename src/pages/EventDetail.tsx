...
const EventDetail: React.FC = () => {
 ...
 <Link
 to="/press-media"
 className="inline-flex items-center gap-2 mb-8 text-sm font-semibold text-gray-500 hover:text-royal-600 transition-colors"
 >
 <ArrowLeft className="w-4 h-4" />
 Back to Press & Media
 </Link>

 <div className="flex items-center gap-3 text-royal-600 mb-4">
 ...
 <h1 className="text-4xl md:text-5xl lg:text-6xl font-black leading-[1.1] tracking-tight text-gray-950">
 {event.title}
 </h1>

 <p className="mt-6 text-lg xl:text-xl leading-relaxed text-gray-600">
 {event.description}
 </p>
 </div>
 </div>
 ...
 <Video className="w-6 h-6 text-royal-600" />
 ...
 <div
 key={media.src}
 className={`overflow-hidden rounded-[2rem] border border-gray-200 bg-black/5 shadow-md transition-transform hover:-translate-y-1 duration-300 ${media.fullWidth ? "sm:col-span-2" : ""}`}
 >
 ...
 <div>
 <div className="flex items-center gap-3 mb-8 text-gray-900">
 <Camera className="w-6 h-6 text-royal-600" />
 <h2 className="text-2xl md:text-3xl font-black">
 Photo Gallery
 </h2>
 </div>
 ...
 className="group relative aspect-square w-full overflow-hidden rounded-2xl bg-gray-100 border border-gray-200 shadow-sm"
 >
 {!loadedImages[src] && (
 ...
 <article
 key={item}
 className="relative aspect-square w-full overflow-hidden rounded-2xl border border-dashed border-gray-300 bg-gray-50"
 aria-label={`Coming soon gallery placeholder ${item}`}
 >
 <div className="absolute inset-0 animate-pulse bg-gradient-to-br from-gray-200 via-gray-100 to-gray-200" />
 <div className="relative z-10 flex h-full w-full items-end p-4">
 <span className="rounded-full bg-white/90 px-3 py-1 text-xs font-bold uppercase tracking-[0.12em] text-gray-700 shadow-sm">
 Coming Soon
 </span>
 </div>
 </article>
